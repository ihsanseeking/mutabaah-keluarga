 ...
 *
 * Keduanya menerima `profile_user_id` opsional: default diri sendiri, atau
 * user_id profil anak (lihat Anggota.js: resolveProfile_) supaya orang tua
 * bisa mengisi checklist atas nama anaknya yang belum punya gadget sendiri.
 */
...
var Checkin = {
  ...
  getState: function (p) {
    ...
    var profile = resolveProfile_(user, p.profile_user_id);
    var keluarga = findRow('keluarga', 'keluarga_id', user.keluarga_id);
    var amalan = Amalan.list(user.keluarga_id, profile.user_id);
    ...
    var checkins = sheetToObjects(getSheet('checkin')).filter(function (c) {
      return c.user_id === profile.user_id && c.tanggal >= since;
    }).map(function (c) {
    ...
    return withLock(function () {
      ...
      items.forEach(function (item) {
        ...
        for (var r = 1; r < data.length; r++) {
          if (data[r][idxUser] === profile.user_id &&
        ...
        var record = {
          ...
          keluarga_id: user.keluarga_id, user_id: profile.user_id, tanggal: item.tanggal,
